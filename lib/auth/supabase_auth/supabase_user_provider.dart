...
class FoodMetricsSupabaseUser extends BaseAuthUser {
  FoodMetricsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> foodMetricsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FoodMetricsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
